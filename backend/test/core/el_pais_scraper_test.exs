defmodule NewspaperScraper.Core.ElPaisScraperTest do
  ...
  setup_all do
    parsed_json = TestUtils.read_and_parse_JSON!("./resources/el_pais_responses.json")
    [search_res: parsed_json]
  end
  ...
  describe "ElPaisScraper.search_articles/3" do
    ...
    test "should not fail when search results have unexpected format" do
      assert {:ok, _} = ElPaisScraper.search_articles("null_resp", 1, 3)
    end
